...
<template>
  <div class="project-root fixed inset-0 flex flex-col bg-zinc-950 text-zinc-100 sm:px-16 overflow-hidden isolate">
    ...
    <!-- Top Header -->
    <header class="w-full shrink-0 z-20 py-2 sm:py-1 sm:pb-1">
      <div class="max-w-7xl mx-auto px-4 sm:px-2 flex justify-between items-end">
    ...
    <div class="flex-1 flex flex-col sm:flex-row overflow-hidden">
      ...
      <aside class="order-2 sm:order-1 w-full sm:w-20 h-auto sm:h-full shrink-0 bg-zinc-950 z-10">
        <!-- flex-1 on the panes inside this flex container will force them to distribute evenly -->
        <nav class="flex flex-row sm:flex-col h-full w-full p-2 gap-2"> 
          <ProjectPane 
      ...
      <!-- Main Viewport -->
      <main class="order-1 sm:order-2 flex-1 relative overflow-y-auto text-slate-950 bg-zinc-900/20 custom-scrollbar">
