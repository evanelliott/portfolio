...
<template>
  <section class="border-2 rounded-md border-zinc-950 bg-zinc-300 overflow-hidden h-full">
    ...
    >
      <div
        class="flex-1 overflow-y-auto p-0 md:p-0 prose prose-zinc prose-sm max-w-none custom-scrollbar scroll-pt-0"
      >
    ...
    <div
      v-else
      class="flex flex-col items-start justify-start h-full px-0 pt-0 text-start"
    >
      <main
        class="flex-1 overflow-y-auto overflow-x-hidden scroll-smooth h-full w-full bg-zinc-300"
      ></main>
    </div>
  </section>
</template>
